...
def eval_once(summary_op, summary_writer, saver, predict_true_or_false):
    with tf.Session() as sess:
        ...
        checkpoint_proto = tf.train.get_checkpoint_state(checkpoint_dir=r'G:\BRL\project\finger\AlexNet_with_tensorflow-master\model/checkpoint')
        ...
        try:
            ...
            # 计算测试数据块的个数,并向上取整
            test_batch_num = math.ceil(Test_number / train.batch_size)
            iter_num = 0
            true_test_num = 0

            total_test_num = test_batch_num * train.batch_size
...
def evaluate():
    with tf.Graph().as_default() as g:
        ...
        img_batch, label_batch = preprocess.read_and_decode('Test.tfrecords',3,flag=False) #读入测试集
